...
class ClampPrimeHook:
    ...
    def __call__(self, module, input, output):
        ...
        # DEBUG: Print hook info (only first few times)
        if not hasattr(self, '_call_count'):
            self._call_count = 0
        self._call_count += 1
        if self._call_count <= 3:
            print(f"\n[DEBUG] Hook called #{self._call_count}")
            print(f"  Output type: {type(output)}, is_tuple: {isinstance(output, tuple)}")
            print(f"  Activations shape: {activations.shape}, device: {activations.device}, dtype: {activations.dtype}")
            print(f"  Activations is_contiguous: {activations.is_contiguous()}")
        ...
        orig_norm = activations.norm().item()
        ...
        if self._call_count <= 3:
            ...
            # Check max activation for target feature
            target_idx = 15864
            print(f"  Feature 15864 max activation: {latents[..., target_idx].max().item():.6f}")
            print(f"  Feature 15864 min activation: {latents[..., target_idx].min().item():.6f}")
        
        # Check if ANY of the harmful features are above threshold
        any_active = torch.zeros(latents.shape[:-1], dtype=torch.bool, device=latents.device)
        for feat_idx in self.harmful_features:
            any_active = any_active | (latents[..., feat_idx] > self.activation_threshold)
        
        if self._call_count <= 3:
            print(f"  Any feature above threshold ({self.activation_threshold}): {any_active.any().item()}")
            print(f"  Num positions with active features: {any_active.sum().item()}")
        
        # If any harmful feature is active, clamp only index 15864
        target_idx = 15864
        latents_before = latents[..., target_idx].clone()
        latents[..., target_idx] = torch.where(
            any_active,
            torch.full_like(latents[..., target_idx], self.clamp_coefficient),
            latents[..., target_idx]
        )
        
        if self._call_count <= 3:
            changed = (latents[..., target_idx] != latents_before).sum().item()
            print(f"  Positions clamped: {changed}")
        ...
        if self._call_count <= 3:
            mod_norm = modified_activations.norm().item()
            diff_norm = (activations - modified_activations).norm().item()
            print(f"  Original norm: {orig_norm:.4f}, Modified norm: {mod_norm:.4f}")
        ...
        if self._call_count <= 3:
            after_copy_norm = activations.norm().item()
            print(f"  After copy_ norm: {after_copy_norm:.4f}")
